...
public class RockSmash : MonoBehaviour
{
    ...
    // Set the direction for the rock smash to travel
    ...
    {
        UnityEngine.Debug.Log("RockSmash collided with: " + collision.gameObject.name);
        TryPushRigidbody(collision.collider);
        TryHitEnemy(collision.transform.root);
        if (collision.gameObject.CompareTag("Wall"))
        {
            UnityEngine.Debug.Log("Collided with wall, destroying RockSmash");
            DestroyRockSmash();
        }
    }
    ...
    // Destroy rock smash and its parent
    private void DestroyRockSmash()
    {
        UnityEngine.Debug.Log("Destroying RockSmash");
